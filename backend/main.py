...
def delete_class(
    ...
    try:
        with closing(get_db()) as conn:
            # 1. Remove samples from database
            conn.execute(
            ...
            # 2. Remove from training_data.pkl and retrain
            ...
            # 3. Retrain model if possible
            user_model = NeuroModel() # Create fresh instance to avoid cache issues
...
async def train_class(
    ...
    try:
        # Save raw samples to database first
        with closing(get_db()) as conn:
...
def delete_class(
    ...
    try:
        # 1. Delete raw samples from the database
        with closing(get_db()) as conn:
            conn.execute(
                "DELETE FROM training_samples WHERE user_id = ? AND class_name = ?",
                (user_id, class_name),
        ...
        # 2. Filter out class from persistent training features
        ...
        # 3. Update the active model instance and file
        user_model = get_user_model(user_id)
        ...
        if len(unique_classes) >= 2:
            ...
            # Invalidate the model if we drop below 2 classes
...
async def connect(sid: str, environ: dict, auth: Any | None = None) -> bool | None:
    ...
    await sio.save_session(sid, {"user_id": int(user_row["id"]), "username": user_row["username"]})
    return None
...
async def join_room(sid: str, data: dict[str, Any]) -> None:
    ...
    log_activity(int(session["user_id"]), "multiplayer", f"Joined multiplayer room {room_id}.")
